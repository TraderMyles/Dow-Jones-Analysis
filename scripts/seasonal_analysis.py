...
====================
Seasonal pattern analysis of the Dow Jones Industrial Average (^DJI).

Period  : 10 years back from today
Ticker  : ^DJI via yfinance (daily OHLCV, auto-adjusted)
...
Outputs
-------
  ../outputs/seasonal_analysis_results.csv
  ../outputs/seasonal_analysis_chart1_dow.png        — day-of-week bar chart
  ../outputs/seasonal_analysis_chart2_month.png      — month bar chart
  ../outputs/seasonal_analysis_chart3_dow_year.png   — day × year heatmap
  ../outputs/seasonal_analysis_chart4_month_year.png — month × year heatmap
"""
...
def main():
    ...
    print(f"\n{'='*65}")
    print(f"  DJIA Seasonal Analysis")
    print(f"  Period: {start_date.date()} → {end_date.date()}")
    print(f"{'='*65}\n")

    # ── Download ───────────────────────────────────────────────────────────────
    print("Downloading ^DJI daily data (10 years)...")
    raw = yf.download(
        TICKER,
    ...
    csv_path = OUTPUTS / "seasonal_analysis_results.csv"
    ...
    save_bar_chart(
        ...
        labels   = DAY_NAMES,
        title    = "DJIA — Average Daily Return by Day of Week (10 Years)",
        xlabel   = "Day of Week",
        ylabel   = "Avg Return % (close-to-close)",
        filepath = OUTPUTS / "seasonal_analysis_chart1_dow.png",
    )
    ...
    save_bar_chart(
        ...
        labels   = MONTH_NAMES,
        title    = "DJIA — Average Daily Return by Calendar Month (10 Years)",
        xlabel   = "Month",
        ylabel   = "Avg Return % (close-to-close)",
        filepath = OUTPUTS / "seasonal_analysis_chart2_month.png",
    )

    # Chart 3 — Day × Year heatmap
    save_heatmap(
        matrix_df  = dow_year,
        title      = "DJIA — Day-of-Week Returns by Year (Avg Daily Return %)",
        filepath   = OUTPUTS / "seasonal_analysis_chart3_dow_year.png",
        fmt        = ".3f",
    ...
    save_heatmap(
        ...
        title      = "DJIA — Monthly Returns by Year (Total Compounded Monthly Return %)",
        filepath   = OUTPUTS / "seasonal_analysis_chart4_month_year.png",
        fmt        = ".2f",
