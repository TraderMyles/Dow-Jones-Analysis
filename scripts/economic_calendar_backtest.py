"""
Script 4 — Economic Calendar Event Backtest on ^DJI

Tests how the Dow Jones Industrial Average reacts around four major
macro-economic release types:

  CPI   — Consumer Price Index  (monthly, BLS ~8:30 AM ET)
  NFP   — Non-Farm Payrolls     (monthly, BLS first Friday ~8:30 AM ET)
  GDP   — Advance GDP estimate  (quarterly, BEA ~8:30 AM ET)
  FOMC  — Fed rate decision     (8× per year, ~2:00 PM ET last meeting day)
...
Note: CPI and GDP dates are hardcoded from BLS/BEA release calendars.
      NFP dates are computed as the first Friday of each month.
      FOMC dates are hardcoded from the Federal Reserve schedule.

Outputs:
  - Terminal summary tables
  - ../outputs/economic_calendar_backtest_results.csv
  - ../outputs/economic_calendar_backtest_chart.png
"""
...
CSV_PATH   = os.path.join(OUTPUT_DIR, 'economic_calendar_backtest_results.csv')
CHART_PATH = os.path.join(OUTPUT_DIR, 'economic_calendar_backtest_chart.png')
...
def first_friday(year, month):
    ...
    offset = (4 - day.weekday()) % 7   # weekday 4 = Friday
...
# BLS CPI release dates (actual published schedule)
CPI_DATES_RAW = [
    # 2021
    '2021-06-10','2021-07-13','2021-08-11','2021-09-14',
    '2021-10-13','2021-11-10','2021-12-10',
    # 2022
...
# BEA Advance GDP release dates (quarterly, ~4 weeks after quarter end)
GDP_DATES_RAW = [
    # Q2 2021 through available 2026
    '2021-07-29','2021-10-28',
...
# Federal Reserve FOMC decision dates (last day of 2-day meeting)
FOMC_DATES_RAW = [
    # 2021
...
EVENT_DATES = {
    ...
    'FOMC': parse_dates(FOMC_DATES_RAW),
}
...
def main():
    ...
    for event_type in ['CPI', 'NFP', 'GDP', 'FOMC']:
    ...
    # ---------------------------------------------------------------------------
    print("\n" + "=" * 110)
    print("DJIA (^DJI) — ECONOMIC CALENDAR EVENT BACKTEST")
    ...
    # --- Panel 1: DJIA price with event date markers ---
    style_ax(ax_price)
    ax_price.plot(close.index, close.values, color='#58a6ff',
                  linewidth=1.0, label='^DJI Close', zorder=3)
    ...
    ax_price.set_title(f'^DJI Close Price with Economic Event Dates',
    ...
    fig.suptitle('DJIA — Economic Calendar Event Backtest (Script 4)',
