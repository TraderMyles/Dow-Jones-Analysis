"""
Script 3 (DAX) — Earnings Announcement Effect Backtest on DAX 40 Components

For each of the 40 DAX 40 stocks, fetches historical quarterly earnings dates
...
Outputs:
  - Terminal summary (strategy aggregates across 40 stocks + top combos)
  - ../outputs/earnings_backtest_dax_results.csv
  - ../outputs/earnings_backtest_dax_chart.png
"""
...
CSV_PATH   = os.path.join(OUTPUT_DIR, 'earnings_backtest_dax_results.csv')
CHART_PATH = os.path.join(OUTPUT_DIR, 'earnings_backtest_dax_chart.png')
...
def main():
    ...
    print(f"Fetching earnings dates for {len(DAX_40)} tickers ...")
    earnings_cache = {}
    ...
    total_tickers = len(DAX_40)
    for ticker_idx, ticker in enumerate(DAX_40):
    ...
    # ---------------------------------------------------------------------------
    print("=" * 105)
    print("DAX 40 — EARNINGS ANNOUNCEMENT EFFECT BACKTEST")
    print(f"Period: {close_df.index[0].date()} to {close_df.index[-1].date()}  "
          f"|  Capital: ${START_CAPITAL:,}/stock  |  Stocks: {len(DAX_40)}")
    print("=" * 105)
    print("\n--- WINDOW STRATEGY AGGREGATES (averaged across 40 stocks) ---\n")
    ...
    for spine in ax_heat.spines.values():
        spine.set_edgecolor(grid_col)
    ax_heat.set_title(
        'Total Return % — Each Earnings Window Strategy × All 40 DAX Stocks',
    ...
    ax_bar.set_title('Average Total Return per Earnings Window Strategy Across 40 Stocks',
    ...
    ax_yr.set_title('Year-by-Year Avg Return per Window (across 40 stocks)',
    ...
    fig.suptitle('DAX 40 — Earnings Announcement Effect Backtest (Script 3)',
