"""
Script 2 — MA Crossover Backtest on All 30 DJIA Component Stocks

Runs the same crossover strategies as Script 1 but on each of the 30
individual Dow Jones component stocks rather than the index itself.
...
Outputs:
  ...
  - ../outputs/ma_backtest_stocks_results.csv
  - ../outputs/ma_backtest_stocks_chart.png
"""
...
CSV_PATH   = os.path.join(OUTPUT_DIR, 'ma_backtest_stocks_results.csv')
CHART_PATH = os.path.join(OUTPUT_DIR, 'ma_backtest_stocks_chart.png')
...
def fetch_all_data():
    ...
    print(f"  Tickers with full data: "
          f"{close_df.notna().all().sum()} / {len(DJIA_30)}")
    return close_df
...
def main():
    ...
    all_rows = []
    total_combos = len(DJIA_30) * (1 + len(CROSSOVER_PAIRS) * len(MA_TYPES))
    done = 0
    ...
    # ---------------------------------------------------------------------------
    print("=" * 100)
    print("DJIA 30 COMPONENTS — MA CROSSOVER BACKTEST SUMMARY")
    print(f"Period: {close_df.index[0].date()} to {close_df.index[-1].date()}  |  "
          f"Capital: ${START_CAPITAL:,} per stock  |  Stocks: {len(DJIA_30)}")
    print("=" * 100)
    print("\n--- STRATEGY AGGREGATES (averaged across all 30 stocks) ---\n")
    ...
    for spine in ax_heat.spines.values():
        spine.set_edgecolor(grid_col)
    ax_heat.set_title('Total Return % Heatmap — All Strategies × All 30 DJIA Stocks',
    ...
    ax_bar.set_title('Average Total Return per Strategy Across All 30 Stocks',
    ...
    for ax, ma_type in [(ax_yr_sma, 'SMA'), (ax_yr_ema, 'EMA')]:
        ...
        ax.set_title(f'{ma_type} — Avg Annual Return % Across 30 Stocks',
    ...
    fig.suptitle('DJIA 30 Components — MA Crossover Backtest (Script 2)',
